 ...
 *
 * @modified Pryymak Slava at 15.04.2015 01:04
 * url: https://github.com/pryymak/PHP-IMAP-CLASS
 */
class ImapMailbox {
	...
	public function getMail($mailId) {
		...
		$mail->dateUnix = isset($head->date) ? strtotime($head->date) : time();
		...
		$mail->priority = $this->getXPriority($header);
		$mail->confirmEmail = $this->getXConfirm($header);
		$mail->dateReceive = $this->getReceiveDate($header);
	...
	/**
	 * Get message X-Priority flag
	 *
	 * @param $header (from getMail() function)
	 * @return priority ( low|normal|high )
	 * Added by Slava: 14.04.2015 11:10
	 */
	protected function getXPriority($header) {
	  if (preg_match('/X-Priority:.*(\d).*/i', $header, $matches) != 0) {
	   if ($matches[1] == 1  ) return "1";
	   else if ($matches[1] == 2  ) return "2";
	   else if ($matches[1] == 4  ) return "4";
	   else if ($matches[1] == 5  ) return "5";
	   else return "3";
	  } else if (preg_match('/X-MSMail-Priority:.*(Low|High|Lowest|Highest).*/i', $header, $matches) != 0) {
           $m = strtolower($matches[1]);
	   if ($m == "highest") return "1";
	   else if ($m == "lowest") return "5";
	   else if ($m == "high") return "2";
	   else if ($m == "low") return "4";
	   else return "3";
	  } else if (preg_match('/Importance:.*(Low|High|Lowest|Highest).*/i', $header, $matches) != 0) {
           $m = strtolower($matches[1]);
	   if ($m == "highest") return "1";
	   else if ($m == "lowest") return "5";
	   else if ($m == "high") return "2";
	...
	/**
	 * Get message Read Confirm email
	 *
	 * @param $header (from getMail() function)
	 * @return email
	 * Added by Slava: 14.04.2015 11:23
	 */
	protected function getXConfirm($header) {
	  if (preg_match('/Return-Receipt-to:(.*):.*/Usmi', $header, $matches) != 0){}
	   else if (preg_match('/Disposition-Notification-To:(.*):.*/Usmi', $header, $matches) != 0){}
	    else if (preg_match('/X-Confirm-Reading-To:(.*):.*/Usmi', $header, $matches) != 0){}
	     else return false;
	  if ( trim($matches[1])!='' && preg_match('~[-a-z0-9_]+(?:\\.[-a-z0-9_]+)*@[-a-z0-9]+(?:\\.[-a-z0-9]+)*\\.[a-z]+~i', trim($matches[1]), $matches2) ) {
	   return  $matches2[0];
	  }
	   return false;
	}
	...
	/**
	 * Get date of receive message
	 *
	 * @param $header (from getMail() function)
	 * @return unixformat datetime
	 * Added by Slava: 14.04.2015 14:13
	 */
	protected function getReceiveDate($header) {
	  if (preg_match('/Received:.*by.*;(.*)\(EEST\).*/Usmi', $header, $matches) != 0) {
	   if ( $date = strtotime(trim($matches[1])) ) return $date;
	   else return time();
	  } else return time();
	}
